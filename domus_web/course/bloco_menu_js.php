...
class BlocoMenuJS {
	...
	function MenuJS($num_id) {
		global $CFG;
		
		$fp = fopen($CFG->dirroot."\blocks\menu_curso\js\menu.js", "w");
		
		for ($i = 0; $i < $num_id; $i++) {
			
			$js = "var XXIEHV$i = { dummy: 0,
				    LAYER: false,
				    X: 0,
				    Y: 0,
				    VALIGN: 0,
				    ...
				    onload: function() { XXIEHV$i.start(); },
				    start: function() {
				        var m = this;
				        m.flyouts = new Array;
				        m.defaultFO = 0;
				    ...
				    over: function(e) {
				        ...
				        if (e) {
				            ...
				            if (e.className == 'PLVFO_UPSCROLLER' || e.className == 'PLVFO_DOWNSCROLLER') {
				                ...
				                m.interval = window.setInterval('XXIEHV$i.scroll(' + e.foid + ')', 35);
				            } else if (e.className == 'PLVFOLink') {
				                m.currentItem = e;
				                m.showMO(e);
				                m.timeout = window.setTimeout('XXIEHV$i.updateFlyouts()', 50);
				            }
				        }
				    },
				    ...
				    out: function(e) {
				        ...
				        window.clearTimeout(m.timeout);
				        m.timeout = window.setTimeout('XXIEHV$i.updateFlyouts()', 150);
				    },
				    ...
				    clicked: function(e) {
				        ...
				        window.clearTimeout(m.timeout);
				        m.updateFlyouts();
				        if (e.func) eval(e.func + '(\"PLVFOMenu script ID:\"+XXIEHV$i.ID,\"' + e.ref + '\",' + e.params + ')');
				        else {
				            if (!e.ref) return;
				...
				XXIEHV$i.flyout = function(m, parent, index) {
				    var f = this;
				    f.index = index;
				    if (index == 0) {
				        f.div = m.div;
				    ...
				    f.m = m;
				    f.div.obj = f;
				    f.obj = 'XXIEHV$i.fo' + index;
				    eval(f.obj + '=f');
				};
				
				XXIEHV$i.flyout.prototype = { showFlyout: function() {
				    var f = this;
				    var m = f.m;
				    var e = f.pfi;
				...
				XXIEHV$i.link = function(m, foi, iti, first, cur, st, i) {
				    var o = this;
				    o.cfoid = st[i++];
				    o.iti = iti;
				    o.txt = st[i++];
