...
class CursosDomus {
	 ...
	 */
	private function MenuPai($array) {
		$indent = '';
		$pai = '';
		$arrTemp = array();
	 ...
	 */
	private function Sequence($id_course, $nome = '') {
	 ...
	 */
	private function SetMenuCache($menu) {
		global $CFG;
	 ...
	 * @return String
	 */
	private function HtmlBlocoMenu($codigo, $titulo, $strm)
	{
		$bloco_id = '';
	 ...
	 * @return String
	 */
	private function BlocoCooperacao()
	{		
		global $CFG, $COURSE;	
		$menu = '';		
	 ...
	 * @return String
	 */
	private function BlocoCursos()
	{
		global $CFG, $COURSE;
		$this -> array_cursos = $arrCurso = array();
		$sql_cursos = "SELECT distinct cu.fullname, cu.id
					        FROM {$CFG->prefix}course cu
					       INNER JOIN {$CFG->prefix}course_categories cat ON cat.id = cu.category
					   	   WHERE cat.id = 4					   	   	 
					         AND cu.visible = 1
					         AND cat.visible = 1
					       ORDER BY cu.sortorder";
		$cursos = get_recordset_sql($sql_cursos);		
		...
		while ($rs_curso = rs_fetch_next_record($cursos)) {
			...
			$strm .= "0," . $cursos -> CurrentRow() . ",'" . $rs_curso -> fullname . "','" . $rs_curso -> fullname . "','javascript:void(0)','','','',0,0,";
		...
		{	
			if($arrCurso[$i] == 25)
			{
				...
				$strm .= "0,0,'Atividades','Atividades','".$CFG->wwwroot."/course/view.php?id=".$arrCurso[$i]."','','','',0,0,
	 ...
	 * @return String
	 */
	private function BlocoConteudos($idCurso)
	{
		global $CFG, $COURSE;		
		$countCurso = 0;
		$this -> array_cursos = array();
	...
	public function ImprimirMenu() {
		...
		if($menu == ""){			
			...
			require_once ("bloco_menu_js.php");			
			new BlocoMenuJS(3);			
