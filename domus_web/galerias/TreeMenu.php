...
class HTML_TreeMenu {
	...
	/**
	 * Arvore com os cursos
	 * @var HTML_TreeMenu_DHTML
	 */
	var $treeMenu;

	/**
	 * Combo com a lista de cursos
	 * @var HTML_TreeMenu_Listbox
	 */
	var $listBox;
	...
	function createTreeMenu($tipo=0) {
		global $CFG;
		
		$icon = 'folder.gif';
		$expandedIcon = 'folder-expanded.gif';
		$cursos = new CursosDomus();
		$tree = array();
		$pai = 0;
		$link = "";

		foreach ($cursos->ImprimirTree() as $key => $row) {

			if ($row['indent'] == -1) {

				if ($key != 0) {
					$this -> addItem($tree[$pai]);
					$tree = array();
				}

				$pai = $row['id'];
				$tree[$pai] = new HTML_TreeNode( array('text' => $row['nome'], 'icon' => $icon, 'expandedIcon' => $expandedIcon));

				array_push($arr_indent[] = array('id' => $pai, 'indent' => $row['indent']));

				continue;
			}
			
			if($tipo == 0){
				$link = "javascript:gerenciarArquivos($pai," . $row['id'] . ",\'" . $row['nome'] . "\');";
			}
			else if($tipo == 1)
			{
				$link = $row["id"];
			...
			$tree[$row['id']] = $tree[$row['pai']] -> addItem(new HTML_TreeNode( array('text' => $row['nome'], 'link' => $link, 'icon' => $icon, 'expandedIcon' => $expandedIcon)));
		}

		$this -> addItem($tree[$pai]);
		
		$this->treeMenu = new HTML_TreeMenu_DHTML($this, array('images' => 'images', 'defaultClass' => 'treeMenuDefault'));
		$this->listBox = new HTML_TreeMenu_Listbox($this, array('linkTarget' => '_self'));
	}
...
class HTML_TreeMenu_Presentation {
	...
	function printCombo($options = array()) {
...
class HTML_TreeMenu_DHTML extends HTML_TreeMenu_Presentation {
	 ...
	 *
	 * @access public
	 * @uses $CFG to access Block-Options for different displays
	...
	function toHTML() {
		...
		global $CFG;
		//Cr: introduced to access different display options
		...
		if ($CFG -> block_siteNavigation_mycourses_withscrollbars) {
			$html = '<div style="width:198px; height:290px; overflow:auto; border:solid 0px">';
		}
		...
		//Besides that there is a footer section in the main block_siteNavigation.php source.
		//The footer is now handled by a config option in the block_siteNavigation.php main source file
		//to display the "all courses" link.

		$enablejs = 'Habilitar Javascript para mostrar navega��o';
		//get_string('enablejs', 'block_siteNavigation');

		//Cr: further enhancement with config setting for scroll bars
		//    The moodle courses page is now displayed in the footer in the main
		//    block_siteNavigation.php according to it's config setting, so I replaced:
		$showallcourses = 'Mostrar todos os cursos';
		//get_string('showallcourses','block_siteNavigation');
		//$html .= "\n// ]]>\n</script><noscript><center>$enablejs</center></noscript></div><center><a href=\"$CFG->wwwroot/course\">$showallcourses</a></center>";
		//    with:
		if ($CFG -> block_siteNavigation_mycourses_withscrollbars) {
			$html .= "\n// ]]>\n</script><noscript><center>$enablejs</center></noscript></div>";
		} else {
			$html .= "\n// ]]>\n</script><noscript><center>$enablejs</center></noscript><center>";
		}
	...
	function _nodeToHTML($nodeObj, $prefix, $return = 'newNode', $currentDepth = 0, $maxDepthPrefix = null) {
		...
		substr(str_replace("'", "\\'", $nodeObj -> text), 0), !empty($nodeObj -> icon) ? "'" . $nodeObj -> icon . "'" : 'null', !empty($nodeObj -> link) ? "'" . $nodeObj -> link . "'" : 'null', $expanded, $isDynamic, $nodeObj -> cssClass, $nodeObj -> linkTarget, !empty($nodeObj -> expandedIcon) ? "'" . $nodeObj -> expandedIcon . "'" : 'null');
...
class HTML_TreeMenu_Listbox extends HTML_TreeMenu_Presentation {
	...
	function toHTML() {
		...
		return sprintf('<form target="%s" action="" onsubmit="var link = this.%s.options[this.%s.selectedIndex].value; if (link) {this.action = link; return true} else return false"><select name="%s"><option value="0">%s</option>%s</select><input type="text" class="txtBuscaGaleria" name="keywords" id="keywords"></input><input type="submit" value="%s" /></form>', $this -> linkTarget, 'HTML_TreeMenu_Listbox_' . ++$count, 'HTML_TreeMenu_Listbox_' . $count, 'HTML_TreeMenu_Listbox_' . $count, $this -> promoText, $nodeHTML, $this -> submitText);
	}
	...
	function toHTML2(){
		...
		//return sprintf('<form target="%s" action="'.$CFG->wwwroot.'/course/jumpto.php" ><select name="jump"><option value="">%s</option>%s</select></input><input type="submit" value="%s" /><input type="hidden" name="sesskey" value=""><div id="noscriptnavmenupopup"></form>', $this -> linkTarget, $this -> promoText, $nodeHTML, $this -> submitText);
		return $nodeHTML; 
