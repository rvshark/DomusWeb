    ...
    this.dataView = new Ext.DataView({
				...
				store: this.store,
                                tpl: new Ext.XTemplate(
                                    '<tpl for=".">',
                                    '<div class="search-item" id="postItem{id}">',
                                        '<h3><span style="width:130px">{updated_at:date("M j, Y H:m:s")}<br /> por {primeiroNome} {ultimoNome}</span>',
                                        '<a href="#" target="_blank"><img height="60px" width="60px"   src="<?php echo $CFG->wwwroot?>/user/pix.php/{userid}/f1.jpg"  /></a> {subject}</h3>',
                                        '<tpl if=" values.citacaoMessage != \'\' ">',                       // 3 
                                            '</br>',
                                            '<div style="display:block" onclick="facadePost.irParaPost({citacaoId})">',
                                        ...
                                        '<div style="text-align:right">',
                                            '<tpl if="this.temAnexo(values)">',
                                                '<a href="#" onclick="facadePost.recuperarAnexo(\'{anexo}\')" target="_blank">{nome_anexo}</a> | ',
                                            '</tpl>',                                                                                    
                                            '<a href="#" onclick="facadePost.responder({id})" target="_blank">Responder</a> ',
                                            '<tpl if="this.podeEditar(values)">',
                                                '| <a href="#" onclick="facadePost.alterarOuCriar({id})" target="_blank">Editar</a> ',
                                                '| <a href="#" onclick="facadePost.excluir(\'{title}\',{id})" target="_blank">Excluir</a>',
                                            '</tpl>', 
                                        '</div>',
                                    '</div></tpl>',{                                                                    // 5 
