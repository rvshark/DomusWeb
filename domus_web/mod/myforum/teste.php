<?php
	global $CFG, $USER;
?>
<script src="jdmenu/jquery-1.3.2.js" language="javascript"></script> 
<script src="jdmenu/jquery.jdMenu.js" language="javascript"></script> 
<script src="jdmenu/jquery.dimensions.js" language="javascript"></script> 
<script src="jdmenu/jquery.bgiframe.js" language="javascript"></script>
<script src="jdmenu/jquery.positionBy.js" language="javascript"></script>
<script type="text/javascript">
	function excluir_post(id,post){
		//alert(id+' '+post);
		//return false;
		if (confirm("Deseja realmente Excluir este post?")){
			location.href="<? echo $CFG->wwwroot?>/mod/myforum/post.php?id="+id+"&acc=delete&post="+post;
			parent.frame_topico.location.reload();
		}
	}
	function interromper_post(id,post){
		if (confirm("Deseja realmente interromper este post?")){
			location.href="<? echo $CFG->wwwroot?>/mod/myforum/post.php?id="+id+"&acc=prune&post="+post;
			parent.frame_topico.location.reload();
		}
	}
	function avaliar(id,post,rate){
		if (rate==""){
			alert("Selecione a nota!");
			return false
		}
		if (confirm("Deseja confirmar?")){
			location.href="<? echo $CFG->wwwroot?>/mod/myforum/post.php?id="+id+"&acc=rating&post="+post+"&rate="+rate;
			//parent.frame_topico.location.reload();
		}
	}
	
	
</script>
<style type="text/css">
body{
	/*font-family:Arial,Verdana,Helvetica,sans-serif;*/
	font-family: Tahoma, sans-serif;
	scrollbar-arrow-color:none !important;
scrollbar-3dlight-color:none !important;
scrollbar-highlight-color:none !important;
scrollbar-face-color:none !important;
scrollbar-shadow-color:none !important;
scrollbar-darkshadow-color:none !important;
scrollbar-track-color:none !important;

}
li{
	list-style:none;
}
a {
	text-decoration:none;
}
#frame_topico{
	width:15%;
	height:450px;
	margin-top:10px;
	border:0px solid #f00;
	
}
#frame_resp{
	width:65%;
	height:450px;
	margin-top:10px;
	
}
#frame_bibli{
	width:80%;
	height:150px;
	
	
}
#menu_lateral{
	border:0px solid #000;
	width:15%; 
	height:618px; 
	float:left;
	
}
.tema{
	font-family: Tahoma, sans-serif;
	background-color: #efefef;
	color: #000;
}
.problema{
	font-family: Tahoma, sans-serif;
	border:1px solid #afafaf;
	background:#fff;
}
.topicos{
	
	background:#efefef;
	width:100%;
	height:100%;
}
.linktopico{
	color: #000;
	padding-left:8px;
	padding-top:0px;
	
}
.cabecalho_forum{
	font-family: Tahoma, sans-serif;
	font-size: 13px;
	border:1px solid #afafaf;
	background:#cfcfcf;
}
.fonte_forum{
	font-family: Tahoma, sans-serif;
	font-size: 12px;
	color:#000;
}
.link{
	font-family: Tahoma, sans-serif;
	font-size: 12px;
	text-decoration: none;
}
ul.menu {
	font-family: Tahoma, sans-serif;
	position: relative;
	margin: 0px;
	padding: 0px;
	width:100%;
	list-style-type: none;
	background-color: #efefef;
	border: 1px solid #70777D;
	
	
}
ul.menu li{
	font-family: Tahoma, sans-serif;
	font-size: 12px;
	color:#000;
	background-color: #FFF;
	padding: 2px 6px 4px 6px;
	padding-top:5px;
	padding-bottom:5px;
	cursor: pointer;
	white-space: pre-line;
	text-decoration: none;
}

ul.menu li a{
	text-decoration: none;
	color:#000;
}

ul.menu_baixo {
	
	height:30px;
	position: relative;
	margin: 0px;
	padding: 0px;
	width:100%;
	list-style-type: none;
	background-color: #fff;
	border: 1px solid #70777D;
	
	
}
ul.menu_baixo li{
	float: left;
	font-family: Tahoma, sans-serif;
	font-size: 14px;
	color:#000;
	background-color: #FFF;
	padding: 2px 6px 4px 6px;
	padding-top:5px;
	padding-bottom:5px;
	cursor: pointer;
	white-space: pre-line;
	text-decoration: none;
}

ul.menu_baixo li a{
	text-decoration: none;
	color:#000;
}


/***************************************************************************************/

ul.jd_menu {
	position: relative;
	margin: 0px;
	padding: 0px;
	list-style-type: none;
	background-color: #FFF;
	border: 1px solid #70777D;
	border-top: 1px solid #A5AFB8;
	border-left: 1px solid #A5AFB8;
	
}

ul.jd_menu a, 
ul.jd_menu a:active,
ul.jd_menu a:link,
ul.jd_menu a:visited
{
	text-decoration: none;
	color: #000;
}
ul.jd_menu li {
	/*float: left;*/
	font-family: Tahoma, sans-serif;
	font-size: 14px;
	padding: 2px 6px 4px 6px;
	cursor: pointer;
	white-space: nowrap;
	
	color: #000;
}
ul.jd_menu li.jd_menu_hover_toolbar {
	padding-left: 5px;
	border-left: 1px solid #ABB5BC;
	padding-right: 5px;
	border-right: 1px solid #929AA1;
	border-right: 1px solid #70777D;
	color: #000;
	/*background: url(gradient-alt.png) repeat-x;*/
}
ul.jd_menu a.jd_menu_hover_toolbar {
	color: #000;
}

/* -- Sub-Menus Styling -- */
ul.jd_menu ul {
	/*position: absolute;
	display: none;*/
	list-style-type: none;
	margin: 0px;
	margin-left:5px;
	padding: 0px;

	background: #fff;
	border: 0px solid #70777D;
}
ul.jd_menu ul li {
	float: none;
	margin: 0px;
	padding: 3px 10px 3px 4px;
	/*width: 115px;*/
	/*background: #E6E6E6;*/
	border: none;
	color: #000;
}
ul.jd_menu ul li.jd_menu_hover {
	/*background: url(gradient.png) repeat-x;*/
	padding-top: 2px;
	border-top: 1px solid #ABB5BC;
	padding-bottom: 2px;
	border-bottom: 1px solid #929AA1;
	/*color: #FFF;*/
}
ul.jd_menu ul a, 
ul.jd_menu ul a:active,
ul.jd_menu ul a:link,
ul.jd_menu ul a:visited {
	text-decoration: none;
	color: #000;
}
ul.jd_menu ul a.jd_menu_hover {
	color: #000;
}

</style>