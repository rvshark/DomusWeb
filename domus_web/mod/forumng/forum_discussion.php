...
class forum_discussion {
     ...
     * @param forum $typeforum If set, this forum is used to potentially restrict
    ...
    static function query_discussions($conditions, $userid, $orderby,
        ...
        if(forum::enabled_read_tracking() && $userid!=-1) {
            ...
            $readtracking = "
                , (CASE WHEN fplast.modified<$deadline THEN " . self::PAST_SELL_BY .
                " ELSE (SELECT COUNT(1)
        ...
        if ($typeforum && $userid != -1) {
            ...
            if ($type->has_unread_restriction() &&
                    $restrictionsql = $type->get_unread_restriction_sql($typeforum, $userid)) {
                $typejoin = "
...
FROM
    ...
    INNER JOIN {$CFG->prefix}user fu ON fpfirst.userid=fu.id
    INNER JOIN {$CFG->prefix}forumng_posts fplast ON fd.lastpostid=fplast.id
    INNER JOIN {$CFG->prefix}user lu ON fplast.userid=lu.id
