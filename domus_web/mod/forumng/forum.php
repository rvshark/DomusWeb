...
SELECT
    x.*, c.id AS courseid, c.shortname AS courseshortname, f.name AS forumname, f.id AS cloneforumid, cm.section AS sectionid
FROM
...
WHERE
    ...
    function updated($previousfields) {
        ...
        if (!$previousfields->shared && $this->is_shared()) {
            //Start sharing
            set_field('forumng_subscriptions', 'clonecmid', $this->get_course_module_id(),
                    'forumid', $previousfields->id);
        }
        if ($previousfields->shared && !$this->is_shared()) {
            //Stop sharing
            set_field('forumng_subscriptions', 'clonecmid', null,
                    'forumid', $previousfields->id);
        }
    }
...
WHERE
    fd.forumid = {$this->forumfields->id}";
        ...
        if ($this->is_clone()) {
            delete_records('forumng_subscriptions', 'clonecmid', $this->get_course_module_id());
        } else {
    ...
    public function get_discussion_list(
        ...
        if ($this->get_type()->has_unread_restriction() &&
                $restrictionsql = $this->get_type()->get_unread_restriction_sql($this, $USER->id)) {
            $querypart = 
...
FROM
    {$CFG->prefix}forumng_discussions fd
    INNER JOIN {$CFG->prefix}forumng_posts fpfirst ON fpfirst.id = fd.postid
    INNER JOIN {$CFG->prefix}forumng_posts fplast ON fplast.id = fd.lastpostid
    INNER JOIN {$CFG->prefix}forumng f ON f.id = fd.forumid
