<?php // $Id: index.php,v 1.35.2.6 2008/10/08 08:25:03 nicolasconnault Exp $
    ...
    if (! $course = get_record("course", "id", $id)) {
		...
		if ($cursos != false) {
			...
			$table->head  = array ("Selecione o conteúdo de <font color='green'>".$category->name."</font> para abrir as atividades");
        	$table->align = array ("left");
			
			while ($rs = rs_fetch_next_record($cursos)) {
				    $table->data[] = array ("<a href='".$CFG->wwwroot."/mod/assignment/index.php?id=" . $rs -> id . "' title='".$rs -> fullname."'>". $rs -> fullname . "</a>");
			}			
