...
class resource_html extends resource_base {
	...
	function display() {
		...
		if ($this->resource->options == 'showblocks') {			
			...
			//	echo '<div style="text-align:center">'.update_course_icon($SITE->id).'</div><br />';
			//}
		...
		else if ($this->resource->options == 'printpage') {
			print_header($pagetitle, $course->fullname, "",
            "", "", false, false,"");
			
			echo '<table id="layout-table" summary="layout" style="width:100%"><tr>';
			
			echo '<td id="">'. skip_main_destination();
			echo format_text($this->resource->alltext, FORMAT_HTML, $formatoptions, $this->course->id);
			echo '</td></tr></table>';
			...
			if ($resource->popup) {
				...
				//echo "<div align='right' ><input type='button' value='Imprimir' onClick='javascript:window.location=\"view2.php?id=$_GET[id]\"' /></div>";//IMPRIME
				echo '<table id="layout-table" summary="layout"><tr>';
				echo '<td style="width: '.$preferred_width_left.'px;" id="left-column">';
				...
				//=====ADD_KIRK======
				if ($_GET[voltar]){
					echo "<br /><br /><div align='right'> <input type='button' onclick='javascript:history.back(-1);' value='Voltar ao Mapa' /> &nbsp;&nbsp;&nbsp;</div>";
				}
				//===================
