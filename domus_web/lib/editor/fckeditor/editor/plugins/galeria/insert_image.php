...
include_once ("../../../../../../galerias/TreeMenu.php");
...
//<![CDATA[
     ...
     window.parent.testando = oEditor;
...
function Init() {
  ...
  if (oImage) {
      var alt = oImage.src.substring(oImage.src.lastIndexOf('/') + 1);
      document.getElementById("f_url").value = oImage.src;
...
/*      document.getElementById("f_border").value = parseInt(param["f_border"] || 0);
      ...
      document.getElementById("f_alt").value = oImage.alt;
      document.getElementById("f_align").value = oImage.align;
      document.getElementById("f_width").value = oImage.width;
      document.getElementById("f_height").value = oImage.height;
      //window.ipreview.location.replace('preview.php?id='+ <?php print($id);?> +'&imageurl='+ alt);
      window.ipreview.location.replace(oImage.src);
  }
...
function onOK() {
  ...
  oImg.alt = param["f_alt"]
  oImg.src = param["f_url"]
  oImg.align = param["f_align"]
  oImg.height = param["f_height"]
  oImg.width = param["f_width"]
...
function onPreview() {
  ...
  var win = null;
  if (!document.all) {
    win = window.open("http://domus.pucpr.br/lib/editor/htmlarea/blank.html", "ha_imgpreview", "toolbar=no,menubar=no,personalbar=no,innerWidth=100,innerHeight=100,scrollbars=no,resizable=yes");
  } else {
    win = window.open("http://domus.pucpr.br/lib/editor/htmlarea/blank.html", "ha_imgpreview", "channelmode=no,directories=no,height=100,width=100,location=no,menubar=no,resizable=yes,scrollbars=no,toolbar=no");
  }
  preview_window = win;
...
<body onload="javascript:Init();">
  <div class="title" style="display: none"><?php print_string("insertimage", "editor"); ?></div>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
  	<tr>
  		<td><fieldset>
    			<legend>Busca Conteúdos</legend>
    			<form method="get" action="<?php echo $CFG-> wwwroot?>/lib/editor/fckeditor/editor/plugins/galeria/insert_image.php">    				
	    			<input type="hidden" id="id" name="id" value="<?php echo $id; ?>" />
	    			<input type="hidden" id="conteudo_id" name="conteudo_id" value="<?php echo $conteudo_id; ?>" />
	    			<?php $comboGalerias -> listBox -> printMenu() ?>
    			</form>
    		</fieldset></td>
  	</tr>
  </table>
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0">
    <tr>
      <td width="55%" valign="top"><?php
		...
		echo "<br />";
		echo "<iframe id=\"ibrowser\" name=\"ibrowser\" src=\"{$CFG->wwwroot}/lib/editor/fckeditor/editor/plugins/galeria/coursefiles.php?usecheckboxes=1&id=$id&conteudo_id=$conteudo_id&keywords=$keywords\" style=\"width: 100%; height: 200px;\"></iframe>";
      ?>
      </td>
 ...
 ?>
  ...
  <script type="text/javascript">
	$(document).ready(function(){		
		...
		$("select[name=HTML_TreeMenu_Listbox_1]").change(function(){
			$("#conteudo_id").val($(this).val());
		})
		
		$("iframe#ibrowser").load(function(){
			$("select[name=HTML_TreeMenu_Listbox_1] option[value='<?php echo $conteudo_id?>']").attr("selected","selected");	
