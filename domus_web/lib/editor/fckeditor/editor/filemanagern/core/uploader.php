...
/** Magic method which allows read-only access to protected or private class properties
  ...
  * @return mixed */
    ...
    public function __construct($diretorio,$usuario_id) {
        ...
        require "config.php";
        $this->config = $_CONFIG;
        $this->config['uploadDir']= $diretorio;
      //  $this->config['uploadURL']= "http://localhost/domus_web/lib/file_sender/2/";
        ...
        // ABSOLUTE & RELATIVE
            ...
            $this->typeDir = "{$this->config['uploadDir']}/{$this->type}";
            $this->config['uploadURL'] = $this->config['uploadURL'] . "/file_sender.php/" . $this->usuario_id;
            $this->typeURL = "{$this->config['uploadURL']}/{$this->type}";
        ...
        if (!is_dir($this->typeDir)) {
            if (!mkdir($this->typeDir, $this->config['dirPerms']))
                $this->backMsg("Cannot create {dir} folder.", array('dir' => $this->type .  $diretorio . 'diretorio' ));
