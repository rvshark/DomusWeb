...
function get_all_instances_in_courses($modulename, $courses, $userid=NULL, $includeinvisible=false) {
	...
	$select_resource_type = $modulename == "resource" ? ", (SELECT rt.name FROM {$CFG->prefix}resource_type rt WHERE rt.id = m.resourcetypeid) as resourcetype" : "";
	
	if (!$rawmods = get_records_sql("SELECT cm.id AS coursemodule, m.*, cw.section, cm.visible AS visible,
                                            cm.groupmode, cm.groupingid, cm.groupmembersonly $select_resource_type
                                       FROM {$CFG->prefix}course_modules cm,
