...
class block_wiki_navigator extends block_base {
    ...
    function init() {
        $this->title = get_string('block_navigator', 'wiki');
    ...
    function get_content() {
    	...
    	foreach ($camefroms as $camefrom){
    		$text.= '<tr><td class="nwikileftnow"><a href="'.$dir.'&amp;page='.urlencode($camefrom->pagename).'">'.format_text($this->trim_string($camefrom->pagename,20),FORMAT_PLAIN).'</a></td></tr>';
    	}
    	$text.='</table>';
    	...
    	foreach ($goestos as $goesto){
    		if (wiki_page_exists($WS,$goesto)){
    			$text.= '<tr><td class="nwikileftnow"><a href="'.$dir.'&amp;page='.urlencode($goesto).'">'.format_text($this->trim_string($goesto,20),FORMAT_PLAIN).'</a></td></tr>';
    		}else{
    			$text.= '<tr><td class="nwikileftnow"><span class="nwikiwanted"><a href="'.$dir.'&amp;page='.urlencode($goesto).'">'.format_text($this->trim_string($goesto,20),FORMAT_PLAIN).'</a></span></td></tr>';
    		}
    	}
    	$text.='</table>';
