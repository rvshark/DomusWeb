...
function email_print_tabs_options($courseid, $folderid, $action=NULL) {
	...
	if ( $CFG->email_enable_ssl ) {
 	...
 	if ( has_capability('block/email_list:sendmessage', $context)) {
		$tabrow[] = new email_tabobject('newmail',   $CFG->httpswwwroot.'/blocks/email_list/email/sendmail.php?course='.$courseid.'&amp;folderid='.$folderid,   get_string('newmail', 'block_email_list'), '<img alt="'.get_string('edit').'" width="15" height="13" src="'. $CFG->pixpath .'/i/edit.gif" />' );
 	}

 	if ( has_capability('block/email_list:createfolder', $context)) {
		$tabrow[] = new email_tabobject('newfolderform', $CFG->httpswwwroot.'/blocks/email_list/email/folder.php?course='.$courseid.'&amp;folderid='.$folderid, get_string('newfolderform', 'block_email_list'), '<img alt="'.get_string('edit').'" width="15" height="15" src="'. $CFG->wwwroot .'/blocks/email_list/email/images/folder_add.png" />' );
 	}
