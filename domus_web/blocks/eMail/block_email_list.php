	...
	function get_content() {
		...
		if ( $this->instance->pageid == 1 ) {
			...
			if (! empty($CFG->mymoodleredirect) and $COURSE->id == 1 ) {
				...
				$this->content->footer .= '<br />'.$composeicon.print_spacer(1,4,false,true).'<a href="'.$CFG->wwwroot.'/blocks/email_list/email/sendmail.php?course='.$course->id.'&amp;folderid=0&amp;filterid=0&amp;folderoldid=0&amp;action=newmail">'.get_string('compose', 'block_email_list').'</a>';
			}
		}
