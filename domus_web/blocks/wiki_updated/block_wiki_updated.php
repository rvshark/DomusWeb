...
class block_wiki_updated extends block_base {
    ...
    function init() {
        ...
        $this->title = get_string('block_updated', 'wiki');
			...
			foreach ($pages as $page){
				...
				$text.= '<tr>
					<td class="nwikileftnow">
						'.$i.'- <a href="'.$dir.'&amp;page='.urlencode($page).'" title="'.$page.'">'.$this->trim_string($page,20).'</a>'.$brs.'
						&nbsp;-&nbsp;<small>('.strftime('%d %b %y',$pageinfo->lastmodified).')</small>
						'.wiki_get_user_info($pageinfo->author,20,false).'
					</td>
					</tr>';
					$i++;
			}
