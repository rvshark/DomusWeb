...
class block_wiki_ead extends block_base {
   ...
   var $pagesel = array();
    ...
    function init() {
        $this->title = get_string('block_ead', 'wiki');
    ...
    function get_content() {
    	...
    	if (wiki_can_change($WS)){
    		foreach ($tools as $tool){
				$text.= '<tr><td class="nwikileftnow">'."\n";
				$text.= "<form id=\"form$i\" action=\"".$tool[1]."\" method=\"post\"><div>"."\n";
				$text.= '<img src="'.$CFG->wwwroot.$basedir.$tool[3].'" alt="" />';
				$text.= ' ';
    		...
    		foreach ($tools_sens as $tool){
    			if ($tool[0]) {
					...
					$text.= '<tr><td class="nwikileftnow">'."\n";
					$text.= "<form id=\"form$i\" action=\"".$tool[2]."\" method=\"post\"><div>";
