...
function initMenu()
{
    ...
    && !( browser.isIE && browser.versionMajor < 5 ) )
    {
		...
		addAnEvent(lis.item(i),'mouseover',show);
                addAnEvent(lis.item(i),'mouseUp',timeoutHide);
...
function show()
{
    ...
    //this.getElementsByTagName('ul')[0].style.background = '#fff';    
...
{	
    ...
    for ( var i=0; i<ul.childNodes.length; i++ )
    {
        if ( ul.childNodes[i].id && ul.childNodes[i].id != currentLi.id )
        {
            ...
            ul.childNodes[i].className="categ"; // enleve marquage du suivi de lien
...
function ancremenu()
{
   ...
   var ancre=document.getElementById('ancre_menu');
  if(ancre){
