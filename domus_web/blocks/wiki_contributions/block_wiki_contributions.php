...
class block_wiki_contributions extends block_base {
  ...
  var $pagesel = array();
    ...
    function init() {
        $this->title = get_string('block_contributions', 'wiki');
    		...
    		foreach ($pages as $page){
    			...
    			$text.= '<tr>
    				<td class="nwikileftnow">
    					<a href="'.$dir.'&amp;page='.urlencode($page).'&amp;gid='.$WS->groupmember->groupid.'&amp;uid='.$WS->member->id.'" title="'.urlencode($page).'">'.format_text($this->trim_string($page,20),FORMAT_PLAIN).'</a>'.$modif.'
    				</td>
    				</tr>';
    		}
    		$text.='</table>';
