...
class block_wiki_index extends block_base {
    ...
    function init() {
        ...
        $this->title = get_string('block_index', 'wiki');
		...
		$this->images = array(
					...
					'square' => '<img src="'.$CFG->wwwroot.'/mod/wiki/images/square.gif" alt="" />'
					);
    }
	...
	function built_index($page,$level){
		...
		if (!in_array($page,$this->pagesel)){
			...
			if (!$pageinfo = wiki_page_last_version ($page)){
				//empty page
				//put in result
				return '<li class="wiki_listme">'.$this->images['square'].format_text($this->trim_string($page,20),FORMAT_PLAIN).'<a href="'.$dir.'&amp;page='.urlencode($page).'">?</a></li>';
			} else {
