...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td width="60%"><input name="f_url" type="text" id="f_url"
			style="width: 100%;" /></td>
	...
	<tr>
		...
		<td align="center">
		<button name="btnCancel" type="button" id="btnCancel"
			onclick="return onCancel();"><?php print_string("cancel","editor") ?></button>
		</td>
	</tr>
</table>
<div class="space"></div>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<fieldset><legend><?php print_string("search","editor") ?> por:</legend>
		<form id="cfolder" action="../files/coursefiles.php" method="post"
			target="ibrowser"><label>Curso:</label> <select name="idSearch"
			id="idSearch">
			<option value="0">Todos</option>
			<?php
			$SQL="select distinct c.fullname, c.id from {$CFG->prefix}course c WHERE category > 0 and visible = 1 ORDER BY c.id";
			$menumembers = get_recordset_sql($SQL);
			while ($rs = rs_fetch_next_record($menumembers)) {
				echo "<option value=\"$rs->id\" >$rs->fullname</option>";
			} ?>
		</select> &nbsp; <label>Tipo Arquivo:</label> <select name="typeFile"
			id="typeFile">
			<option value="all">Todos</option>
			<option value="img">Imagens</option>
			<option value="docs">Documentos</option>
		</select> &nbsp; <label>Palavra:</label> <input type="text"
			name="keywords" id="keywords" value="" /> <input type="hidden"
			name="id" value="<?php print($id); ?>" /> <input type="hidden"
			name="images" value="1" /> <input type="hidden" name="action"
			value="search" /> <input name="name" type="submit" id="foldername"
			value="Buscar" /></form>
		</fieldset>
		</td>
	</tr>
	<tr>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td width="60%"><select size="1" name="f_align" id="f_align"
					...
					<option value="baseline" selected="selected"><?php print_string("baseline","editor") ?></option>
			...
			<tr>
				...
				<td width="60%"><input type="text" id="f_border" name="f_border"
					size="5" /></td>
			</tr>
		</table>
		<div class="space"></div>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				<td width="40%" align="right"><?php print_string("horizontal","editor");?>:
				</td>
				<td width="60%"><input name="f_horiz" type="text" id="f_horiz"
					size="5" /></td>
			</tr>
			<tr>
				<td width="40%" align="right"><?php print_string("vertical","editor");?>:</td>
				<td width="60%"><input name="f_vert" type="text" id="f_vert"
					size="5" /></td>
			</tr>
		</table>
		<div class="space"></div>
		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0">
			<tr>
				...
				<td width="60%"><input name="f_width" type="text" id="f_width"
					size="5" /></td>
			</tr>
			<tr>
				<td width="40%" align="right"><?php print_string("height","editor");?>:</td>
				<td width="60%"><input name="f_height" type="text" id="f_height"
					size="5" /></td>
			</tr>
		</table>
		<div class="space"></div>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		echo "<br />";
		//echo "<iframe id=\"ibrowser\" name=\"ibrowser\" src=\"{$CFG->wwwroot}/files/coursefiles.php?usecheckboxes=1&id=$id&images=1\" style=\"width: 100%; height: 200px;\"></iframe>";
		echo "<iframe id=\"ibrowser\" name=\"ibrowser\" src=\"{$CFG->wwwroot}/files/coursefiles.php?usecheckboxes=1&id=$id&images=\" style=\"width: 100%; height: 200px;\"></iframe>";
		?></td>
		<td width="45%" valign="top"><?php print_string("preview","editor");?>:<br />
		<iframe id="ipreview" name="ipreview"
			src="<?php echo $url ?>blank.html"
			style="width: 100%; height: 200px;"></iframe></td>
	</tr>
</table>
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<table border="0" cellpadding="2" cellspacing="0">
			<tr>
				...
				<form id="imove"><input name="btnMove" type="submit" id="btnMove"
					...
					onclick="return submit_form('move');" /></form>
				</td>
				<td>
				<form id="izip"><input name="btnZip" type="submit" id="btnZip"
					value="<?php print_string("zip","editor");?>"
					onclick="return submit_form('zip');" /></form>
				</td>
			</tr>
		</table>
		<br />
		...
		<fieldset><legend><?php print_string("properties","editor");?></legend>
		<div class="space"></div>
		<div class="space"></div>
		&nbsp;&nbsp;<?php print_string("size","editor");?>: <input type="text"
			id="isize" name="isize" size="10"
			style="background: transparent; border: none;" /> <?php print_string("type","editor");?>:
		<input type="text" id="itype" name="itype" size="10"
	...
	<tr>
		<td height="22">
		<?php if(has_capability('moodle/course:managefiles', get_context_instance(CONTEXT_COURSE, $id))) { ?>
		<form id="cfolder" action="coursefiles.php" method="post"
			target="ibrowser"><input type="hidden" name="id"
			value="<?php print($id);?>" /> <input type="hidden" name="wdir"
			value="" /> <input type="hidden" name="action" value="mkdir" /> <input
			type="hidden" name="sesskey" value="<?php p($USER->sesskey) ?>" /> <input
			name="name" type="text" id="foldername" size="35" /> <input
			name="btnCfolder" type="submit" id="btnCfolder"
			value="<?php print_string("createfolder","editor");?>"
			onclick="return checkvalue('foldername','cfolder');" /></form>
		<div class="space"></div>
		<?php
		} else {
			print "";
		} ?>
		<form action="coursefiles.php?id=<?php print($id);?>" method="post"
			enctype="multipart/form-data" target="ibrowser" id="uploader">
			<input type="hidden" name="MAX_FILE_SIZE" value="<?php print($upload_max_filesize);?>" /> 
			<input type="hidden" name="id" value="<?php print($id);?>" /> 
			<input type="hidden" name="wdir" value="upload" /> 
			<input type="hidden" name="action" value="upload" /> 
			<input type="hidden" name="sesskey" value="<?php p($USER->sesskey) ?>" /> 
			<input type="file" name="userfile" id="userfile" size="35" /> 
			<input name="save" type="submit" id="save" onclick="return checkvalue('userfile','uploader');"
			value="<?php print_string("upload","editor");?>" /></form>
		</td>
	</tr>
</table>
<p>&nbsp;</p>
